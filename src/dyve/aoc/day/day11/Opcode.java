...
public class Opcode {
    ...
    public void compute(Program p){
        switch (operationId){
            ...
            case 3 -> {
                int input = p.robot.getInput();
                p.write((int) params[0].readPosition(p), input);
                System.out.println("Write input " + input + " at " + p.read((int) params[0].readPosition(p)));
            }
            case 4 -> {
                p.robot.readOutput((int) params[0].readParam(p));
